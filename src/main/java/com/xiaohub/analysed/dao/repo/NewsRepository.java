...
public interface NewsRepository extends BaseRepository<News, Long> {
    ...
    @Query(value = "SELECT * FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY platform ORDER BY publication_date DESC) AS rn FROM t_news) AS ranked WHERE rn <= 10", nativeQuery = true)
